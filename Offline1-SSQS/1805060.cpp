...
public:
    // Constructor
    SingleServerQueueSimulator(const char* in_filename, const char* out_filename, const char* out_filename2)
...
void SingleServerQueueSimulator::initialize() {
    sim_time = 0.0;
    server_status = IDLE;
    num_in_q = 0;
    event_count = 0;
...
float SingleServerQueueSimulator::expon(float mean) {
    return -mean * log(lcgrand(1));
}

void SingleServerQueueSimulator::timing() {
    event_count++;
...
void SingleServerQueueSimulator::update_time_avg_stats() {
...
void SingleServerQueueSimulator::arrive() {
    float delay;
    next_arrival_time = sim_time + expon(A);
...
void SingleServerQueueSimulator::depart() {
    float delay;
    if (num_in_q == 0) {
        server_status = IDLE;
        next_departure_time = numeric_limits<float>::infinity();
...
void SingleServerQueueSimulator::report() {
...
void SingleServerQueueSimulator::simulate() {
    ifstream infile;
    ...
    infile >> A >> S >> N; // the mean inter-arrival time, the mean service time and the
...
// Main function
int main() {
    SingleServerQueueSimulator simulator("in.txt", "results.txt", "event_orders.txt");
    simulator.simulate();
    return 0;
}